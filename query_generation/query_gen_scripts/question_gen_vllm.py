...
for dataset in beir_datasets:
    ...
    gpt_examples = []
    with open('../prompt_examples/gpt_examples_questions.json', 'r', encoding='utf-8') as input_pairs:
        gpt_examples = json.load(input_pairs)

    system_prompt = 'You are tasked with creating a relevant question that can be answered by a given passage.'
    user_prompt = '''Instructions: Read the target passage carefully to understand its main idea. Then, write a natural-sounding question based on the passage, ensuring that it is directly addressed by the passage and can stand alone without additional context. The question should be clear, concise, and under 20 words. Do not copy long phrases or sections directly from the passage. Respond only with the plain text question.
    Examples:
    ...
    Now create a question for the following passage:
    Target Passage: {}
    Question: 
    '''
    ...
    index = 0
    with open('../generated_queries/' + dataset + '_generated_queries_questions.tsv', 'w') as f:

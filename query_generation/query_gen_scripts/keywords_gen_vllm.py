...
for dataset in beir_datasets:
    ...
    gpt_examples = []
    with open('../prompt_examples/gpt_examples_keywords.json', 'r', encoding='utf-8') as input_pairs:
        gpt_examples = json.load(input_pairs)

    system_prompt = 'You are tasked with generating up to 5 relevant keywords or key phrases based on a given passage.'
    user_prompt = '''Instructions: Read the target passage carefully to understand its main idea. Then, write up to 5 concise keywords or key phrases that represent important topics, concepts, or themes related to the passage. Focus on using general terms that represent the passage's content, rather than directly copying phrases from it. Avoid being repetitive. Respond only with the plain text keywords or key phrases, separated by commas.
    Examples:
    ...
    Now create keywords or key phrases for the following passage:
    Target Passage: {}
    Keywords/Phrases: 
    '''
